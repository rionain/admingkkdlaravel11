                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card-box table-responsive">
                            <div class="row">
                                <div class="col-sm-6">
                                    <button id="tambahIbadah" class="btn btn-success waves-effect waves-light" data-toggle="modal" data-target="#kehadiranIbadah">{{ addText() }} <i class="mdi mdi-plus-circle-outline"></i></button>
                                </div>
                                <div class="col-sm-6">
                                    <form action="" id="formCariKehadiranIbadah">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-4" style="margin-top: 10px;">
                                                <div class="form-group">
                                                    <div class="input-group">
                                                        <input type="text" name="tgl_akhir" id="tgl_akhir" class="form-control" placeholder="Tanggal akhir" value="{{ request('tgl_akhir') }}" readonly>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($kehadiran as $key => $value)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-right">{{ format_angka($value->jumlah_wanita) ?: 0 }}</td>
                                            <td class="text-right">{{ format_angka($value->jumlah_pria_baru) ?: 0 }}</td>
                                            <td class="text-right">{{ format_angka($value->jumlah_wanita_baru) ?: 0 }}</td>
                                            <td class="text-right">{{ format_angka($value->jumlah_pria + $value->jumlah_wanita + $value->jumlah_pria_baru + $value->jumlah_wanita_baru) ?: 0 }}</td>
                                            <td class="text-right">Rp. {{ format_angka($value->persembahan ?: 0) }}</td>
                                            <td class="actions table-action text-center">
                                                <a href="#" class="btn btn-primary btn-action" data-toggle="modal" data-target="#modalDetailIbadah" onclick="detailKehadiranIbadah('{{ $value->ibadah_detail_id }}')">
                                                    <i class="fa fa-eye"></i> {{ detailText() }}
                                                </a>
                                                <a href="#" class="btn btn-warning btn-action" data-toggle="modal" data-target="#kehadiranIbadah" onclick="editKehadiranIbadah('{{ $value->ibadah_detail_id }}')">
                                                    <i class="fa fa-pencil"></i> {{ editText() }}
                                                </a>
                                                <a href="#" class="btn btn-danger btn-action" onclick="delConf('{{ url('admin-cabang/kehadiran/ibadah/hapus/' . $value->ibadah_detail_id) }}')">
                ...
                <div id="kehadiranIbadah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content ">
                            ...
                            <form action="{{ url('/admin-cabang/kehadiran/ibadah') }}" method="POST"
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group no-margin">
                                        ...
                                        <textarea class="form-control autogrow" id="catatan" name="catatan"
                                ...
                                <div class="modal-footer">
                                    ...
                                    <div class="btn btn-success waves-effect waves-light" onclick="submitForm()">Simpan</div>
                                </div>
                            </form>
...
@section('script')
    ...
    <script>
        ...
        $('#'+modalKehadiranIbadah+' #persembahan').on('input', function() {
            if($('#'+modalKehadiranIbadah+' #persembahan').val() === '' || $('#'+modalKehadiranIbadah+' #persembahan').val() === null || $('#'+modalKehadiranIbadah+' #persembahan').val() === 'Rp. ') {
                $('#'+modalKehadiranIbadah+' #persembahan').val('')
            } else {
                $('#'+modalKehadiranIbadah+' #persembahan').val(formatRupiah($('#'+modalKehadiranIbadah+' #persembahan').val()))
            }
        })

        $('#tambahIbadah').on('click', function() {
            titleAction('Tambah Kehadiran Ibadah', base_url('admin-cabang/kehadiran/ibadah'))
        })
        ...
        $('#' + modalKehadiranIbadah).on('hidden.bs.modal', function() {
            ...
            setVal(modalKehadiranIbadah, 'tempat_ibadah', '')
        })
        ...
        function editKehadiranIbadah(id) {
            titleAction('Edit Kehadiran Ibadah', base_url('admin-cabang/kehadiran/ibadah/edit/' + id))
            startloading('#' + modalKehadiranIbadah + ' .modal-dialog')
            var settings = {
                'url': base_url('api/v1/admin-cabang/kehadiran/ibadah/' + id),
                'method': 'GET',
                'dataType': 'json',
                'timeout': timeOut()
            };
            $.ajax(settings).done(function(response) {
                ...
                setValSelect2(modalKehadiranIbadah, 'lfk_kategori_gereja_id', response.data.ibadah.cabang.lfk_kategori_gereja_id)
                ...
                setVal(modalKehadiranIbadah, 'tempat_ibadah', response.data.tempat_ibadah)
        ...
        function submitForm() {
            $('#'+modalKehadiranIbadah+' #persembahan').val(removeSpace($('#'+modalKehadiranIbadah+' #persembahan').val()))
