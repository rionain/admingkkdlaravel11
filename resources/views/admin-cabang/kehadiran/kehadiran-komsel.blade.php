                ...
                <div class="row">
                    <div class="panel">
                        <div class="panel-body">
                            ...
                            <div id="kehadiranKomsel" class="modal fade" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ url('/admin-cabang/kehadiran/komsel') }}" method="POST"
                                            enctype="multipart/form-data" id="form-kehadiran-komsel">
                                            @csrf
                                            <div class="modal-body">
                                                <div class="form-group no-margin">
                                                    <label for="lfk_komsel_id" class="control-label">Komsel</label>
                                                    <select class="form-control autogrow" id="lfk_komsel_id"
                                                        name="lfk_komsel_id">
                                                        <option value="">Pilih komsel</option>
                                                        @foreach ($komsel as $item)
                                                            <option value="{{ $item->komsel_id }}"
                                                                {{ $item->komsel_id == old('lfk_komsel_id') ? 'selected' : '' }}>
                                                                {{ $item->nama_komsel }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="form-group no-margin">
                                                    ...
                                                    <input type="date" class="form-control autogrow" id="komsel_date" name="komsel_date" value="{{ old('komsel_date') ?: date('Y-m-d') }}">
                                                </div>
                                                <div class="form-group no-margin">
                                                    <label for="jumlah_pria" class="control-label">Jumlah Pria</label>
                                                    <input type="number" min="0" minlength="1" class="form-control autogrow"
                                                        id="jumlah_pria" name="jumlah_pria" placeholder="Jumlah Pria"
                                                        value="{{ old('jumlah_pria') }}">
                                                </div>
                                                <div class="form-group no-margin">
                                                    <label for="jumlah_wanita" class="control-label">Jumlah Wanita</label>
                                                    <input type="number" min="0" minlength="1" class="form-control autogrow"
                                                        id="jumlah_wanita" name="jumlah_wanita" placeholder="Jumlah Wanita"
                                                        value="{{ old('jumlah_wanita') }}">
                                                </div>
                                                <div class="form-group no-margin">
                                                    <label for="jumlah_pria_baru" class="control-label">Jumlah Pria
                                                        Baru</label>
                                                    <input type="number" min="0" minlength="1" class="form-control autogrow"
                                                        id="jumlah_pria_baru" name="jumlah_pria_baru"
                                                        placeholder="Jumlah Pria Baru"
                                                        value="{{ old('jumlah_pria_baru') }}">
                                                </div>
                                                <div class="form-group no-margin">
                                                    <label for="jumlah_wanita_baru" class="control-label">Jumlah Wanita
                                                        Baru</label>
                                                    <input type="number" min="0" minlength="1" class="form-control autogrow"
                                                        id="jumlah_wanita_baru" name="jumlah_wanita_baru"
                                                        placeholder="Jumlah Wanita Baru"
                                                        value="{{ old('jumlah_wanita_baru') }}">
                                                </div>
                                                <div class="form-group no-margin">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <button id="tambahKomsel" class="btn btn-success waves-effect waves-light"
                                        data-toggle="modal" data-target="#kehadiranKomsel">Add <i
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($kehadiran as $key => $value)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">{{ format_date($value->komsel_date, 'd F Y') }}</td>
                                                <td class="text-center">{{ $value->jumlah_pria }}</td>
                                                <td class="text-center">{{ format_angka($value->jumlah_wanita) }}</td>
                                                <td class="text-center">{{ format_angka($value->jumlah_pria_baru) }}</td>
                                                <td class="text-center">{{ format_angka($value->jumlah_wanita_baru) }}</td>
                                                <td class="text-center">{{ format_angka($value->jumlah_pria + $value->jumlah_wanita + $value->jumlah_pria_baru + $value->jumlah_wanita_baru) }}</td>
                                                <td>{{ $value->catatan }}</td>
                                                <td class="text-center table-action">
                                                    <a href="#" class="btn btn-warning btn-action" data-toggle="modal" data-target="#kehadiranKomsel" onclick="editKehadiranKomsel('{{ $value->komsel_detail_id }}')">
                                                        <i class="fa fa-pencil"></i> {{ edittext() }}
                                                    </a>
                                                    <a href="{{ url('admin-cabang/kehadiran/komsel/hapus/' . $value->komsel_detail_id) }}" class="btn btn-danger btn-action" onclick="return confirm('Apakah anda yakin?')">
...
@section('script')
    ...
    <script>
        ...
        function editKehadiranKomsel(id) {
            titleAction('Form Edit', base_url('admin-cabang/kehadiran/komsel/' + id))
            startloading('#' + kehadiranKomsel + ' .modal-dialog')
            var settings = {
                'url': base_url('api/v1/admin-cabang/kehadiran/komsel/' + id),
                'method': 'GET',
                'dataType': 'json',
                'timeout': timeOut()
            };
