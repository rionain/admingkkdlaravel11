...
@section('content')
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card-box table-responsive">
                            <h4 class="m-t-0 header-title"><b>Surat tugas</b></h4>
                            <p class="text-muted font-13 m-b-30"></p>
                            {{-- <div class="table-responsive"> --}}
                            <table class="{{ styletable() }}" id="datatable-surattugas">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @foreach ($surat_tugas as $key => $item)
                                        <tr class="">
                                            ...
                                            <td class="actions text-center">
                                                <button class="btn btn-primary" style="margin-bottom: 5px;"
                                                    onclick="window.open('{{ url('superadmin/administrasi/surat/surat-tugas/lihat/' . $item->surat_tugas_id, []) }}', 'newwindow', 'width=1000,height=700,left=450'); return;">
                                                    <i class="fa fa-eye"></i>
                                                    Lihat</button>
                                                @if ($item->status_surat_id == 6)
                                                    <a href="{{ url('superadmin/administrasi/surat/pdf/' . $item->surat_tugas_id, []) }}"
                                                ...
                                                @else
                                                    @if ($item->pengaturan_surat_tugas_id)
                                                        <button class="btn btn-success" style="margin-bottom: 5px;"
                                                            ...
                                                            onclick="ubahStatus('{{ $item->surat_tugas_id }}')"><i
                                                                class="fa fa-edit"></i>
                                                            Status</button>
                                                    @endif
                                                    <button class="btn btn-success" style="margin-bottom: 5px;"
                                                        data-toggle="modal" data-target="#settingTemplate"
                                                        onclick="showSettingTemplate('{{ $item->surat_tugas_id }}')"><i
                                                            class="fa fa-edit"></i>
    ...
    <div id="settingTemplate" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <form action="{{ url('superadmin/administrasi/surat/surat-tugas/edit-master-surat/') }}" method="POST"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="pengaturan_surat_tugas_id">Master Surat</label><br>
                            <select id="pengaturan_surat_tugas_id" name="pengaturan_surat_tugas_id" class="form-control">
                                <option value="">- Pilih master surat -</option>
                                @foreach ($pengaturan_surat_tugas as $item)
                                    <option value="{{ $item->pengaturan_surat_tugas_id }}">
                        ...
                        <div class="form-group">
                            <label for="tempat_surat">Tempat surat</label><br>
                            <input type="text" name="tempat_surat" id="tempat_surat" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label for="tanggal_surat">Tanggal surat</label><br>
...
@section('script')
    ...
    {{-- <script src="{{ url('assets/pages/jquery.datatables.surattugas.init.js') }}"></script> --}}
    ...
    <script>
        $('#datatable-surattugas').DataTable({});
        ...
        function ubahStatus(id) {
            titleActionStatusSurat('Edit status', base_url('superadmin/administrasi/surat/surat-tugas/ubah-status/' +
                id + '?'))
            startloading('#' + modalBody + ' .modal-dialog')

            let data = {};
            var settings = {
                'url': base_url('api/v1/superadmin/administrasi/surat/surat-tugas/' + id),
                'method': 'GET',
                'dataType': 'json',
                'timeout': timeOut()
            };
    ...
    <script>
        ...
        $('#' + modalSettingTemplate).on('hidden.bs.modal', function() {
            ...
            tutupModalSettingTemplate(modalSettingTemplate, 'pengaturan_surat_tugas')
        });

        function showSettingTemplate(id) {
            titleActionSettingTemplate('Setting Master Surat', base_url(
                'superadmin/administrasi/surat/surat-tugas/ubah-template/' + id))
            ...
            $.ajax(settings).done(function(response) {
                ...
                setVal(modalSettingTemplate, 'pengaturan_surat_tugas_id', response.data
                    .pengaturan_surat_tugas_id)
                setVal(modalSettingTemplate, 'tempat_surat', response?.data?.tempat_surat)
                setVal(modalSettingTemplate, 'tanggal_surat', response?.data?.tanggal_surat)
