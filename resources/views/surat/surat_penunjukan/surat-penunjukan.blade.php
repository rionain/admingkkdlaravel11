...
@section('content')
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card-box table-responsive">
                            ...
                            <table class="{{ styletable() }}" id="datatable-suratpenunjukan">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @foreach ($surat_penunjukan as $key => $item)
                                        <tr class="">
                                            ...
                                            <td class="actions text-center">
                                                <button class="btn btn-primary" style="margin-bottom: 5px;"
                                                    onclick="window.open('{{ url('superadmin/administrasi/surat/surat-penunjukan/lihat/' . $item->surat_penunjukan_id, []) }}', 'newwindow', 'width=1000,height=700,left=450'); return;">
                                                    <i class="fa fa-eye"></i>
                                                    Lihat</button>
                                                @if ($item->status_surat_id == 6)
                                                    {{-- <a href="{{ url('superadmin/administrasi/surat/pdf/' . $item->surat_penunjukan_id, []) }}"
                                                ...
                                                @else
                                                    @if ($item->pengaturan_surat_penunjukan_id)
                                                        <button class="btn btn-success" style="margin-bottom: 5px;"
                                                            ...
                                                            onclick="ubahStatus('{{ $item->surat_penunjukan_id }}')"><i
                                                                class="fa fa-edit"></i>
                                                            Status</button>
                                                    @endif
                                                    <button class="btn btn-success" style="margin-bottom: 5px;"
                                                        data-toggle="modal" data-target="#settingTemplate"
                                                        onclick="showSettingTemplate('{{ $item->surat_penunjukan_id }}')"><i
                                                            class="fa fa-edit"></i>
    ...
    <div id="settingTemplate" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <form action="{{ url('superadmin/administrasi/surat/surat-penunjukan/edit-master-surat/') }}" method="POST"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="pengaturan_surat_penunjukan_id">Master Surat</label><br>
                            <select id="pengaturan_surat_penunjukan_id" name="pengaturan_surat_penunjukan_id"
                                class="form-control">
                                <option value="">- Pilih master surat -</option>
                                @foreach ($pengaturan_surat_penunjukan as $item)
                                    <option value="{{ $item->pengaturan_surat_penunjukan_id }}">
                        ...
                        <div class="form-group">
                            <label for="tempat_penunjukan">Tempat penunjukan</label><br>
                            <input type="text" name="tempat_penunjukan" id="tempat_penunjukan" class="form-control"
                                required>
                        </div>
                        <div class="form-group">
                            <label for="tanggal_penunjukan">Tanggal penunjukan</label><br>
                            <input type="date" name="tanggal_penunjukan" id="tanggal_penunjukan" class="form-control"
                                required>
                        </div>
                    </div>
                    <div class="modal-footer">
...
@section('script')
    ...
    {{-- <script src="{{ url('assets/pages/jquery.datatables.suratpenunjukan.init.js') }}"></script> --}}
    ...
    <script>
        $('#datatable-suratpenunjukan').DataTable({});
        ...
        function ubahStatus(id) {
            titleActionStatusSurat('Edit status', base_url('superadmin/administrasi/surat/surat-penunjukan/ubah-status/' +
                id + '?'))
            startloading('#' + modalBody + ' .modal-dialog')

            let data = {};
            var settings = {
                'url': base_url('api/v1/superadmin/administrasi/surat/surat-penunjukan/' + id),
                'method': 'GET',
                'dataType': 'json',
                'timeout': timeOut()
            };
    ...
    <script>
        ...
        $('#' + modalSettingTemplate).on('hidden.bs.modal', function() {
            ...
            tutupModalSettingTemplate(modalSettingTemplate, 'pengaturan_surat_penunjukan')
        });

        function showSettingTemplate(id) {
            titleActionSettingTemplate('Setting Master Surat', base_url(
                'superadmin/administrasi/surat/surat-penunjukan/ubah-template/' + id))
            ...
            $.ajax(settings).done(function(response) {
                ...
                setVal(modalSettingTemplate, 'pengaturan_surat_penunjukan_id', response.data
                    .pengaturan_surat_penunjukan_id)
                setVal(modalSettingTemplate, 'tempat_penunjukan', response?.data?.tempat_penunjukan)
                setVal(modalSettingTemplate, 'tanggal_penunjukan', response?.data?.tanggal_penunjukan)
